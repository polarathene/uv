...
#[derive(Debug)]
pub struct Pep508Error<T: Pep508Url = Url> {
...
/// Either we have an error string from our parser or an upstream error from `url`
#[derive(Debug, Error)]
pub enum Pep508ErrorSource<T: Pep508Url = Url> {
...
pub struct Requirement<T: Pep508Url = Url> {
...
impl<T: Pep508Url> Requirement<T> {
    ...
    pub fn convert_url<U: Pep508Url>(self) -> Requirement<U>
    where
        U: From<T>,
    {
        let Requirement {
...
#[derive(Debug, Clone, Eq, Hash, PartialEq)]
pub enum VersionOrUrl<T: Pep508Url = Url> {
...
#[derive(Debug, Clone, Eq, Hash, PartialEq)]
pub enum VersionOrUrlRef<'a, T: Pep508Url = Url> {
    /// A PEP 440 version specifier set
...
mod tests {
    ...
    use crate::verbatim_url::VerbatimUrl;
    use crate::{Requirement, VersionOrUrl};
